...
FluorinatedInfoDialog::FluorinatedInfoDialog(QWidget *parent)
  ...
  addInfoSection(
      tr("What are Fluorinated Compounds?"),
      tr("Fluorinated compounds include a series of fluorinated chemical "
         ...
         "chemical properties, but they also bring about environmental "
         "problems."));

  addInfoSection(tr("Environmental Persistence"),
                 tr("• Extremely slow degradation in the environment\n"
                 "• Have bioaccumulative properties\n"
                 "• Can migrate long distances in environmental media"));

  addInfoSection(tr("Potential Impacts"),
                 tr("• May affect the endocrine systems of organisms\n"
                 "• May impact immune functions\n"
                 "• Pose threats to aquatic ecosystems\n"
                 "• Bioaccumulate through the food chain"));

  addInfoSection(tr("Monitoring Standards"),
                 QString(tr("The current safety threshold is %1 μg/L. Measures "
...
FluorinatedCompoundsPage::FluorinatedCompoundsPage(QWidget *parent)
  ...
  connect(&Dataset::instance(), &Dataset::dataUpdated, this,
          &FluorinatedCompoundsPage::updateLocationList);
...
void FluorinatedCompoundsPage::updateLocationList() {
      ...
      if (!location.isEmpty()) {
        ...
        if (!std::isnan(value) && value >= 0) {
          ...
          qDebug() << "Location:" << location
                   << "Count:" << locationDataCount[location];
        }
      }
    }
  }
  ...
  for (auto it = locationDataCount.constBegin();
       it != locationDataCount.constEnd(); ++it) {
    if (it.value() > 2) { // 改为大于2个数据点
      locationList.append(it.key());
      qDebug() << "Added location:" << it.key() << "with" << it.value()
               << "data points";
    }
  }
...
void FluorinatedCompoundsPage::processData() {
  ...
  if (selectedLocation.isEmpty() ||
      selectedLocation == tr("No data available")) {
  ...
  if (tempData.size() > 2) {
    ...
    qDebug() << "Processed" << compoundData.size()
             << "data points for charting";
  } else {
    qDebug() << "Not enough data points:" << tempData.size()
             << "found (need >2)";
  }
}
