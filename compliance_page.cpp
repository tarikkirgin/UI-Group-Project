...
compliancePage::compliancePage() : QWidget() {
  ...
  setupUI();
  // qDebug() << "B";
  connect(pollutantComboBox,
          &QComboBox::currentTextChanged, // takes the string value of text
                                          // that's changed
  ...
  connect(complianceComboBox, &QComboBox::currentTextChanged,
          &PollutantDataset::instance(),
          &PollutantDataset::onComplianceChanged);
  // qDebug() << "D";
  connect(&LocationDataset::instance(), &LocationDataset::dataUpdated, this,
          &compliancePage::updateDropdownChart);
  // qDebug() << "E";
  connect(pollutantComboBox, &QComboBox::currentTextChanged, this,
          &compliancePage::getCardUI);
  connect(complianceComboBox, &QComboBox::currentTextChanged, this,
          &compliancePage::getCardUI);
}
...
void compliancePage::updateDropdownChart() {
  ...
  for (const auto &pollutant : LocationDataset::instance().data) {
    pollutantList << QString::fromStdString(
        pollutant.getDeterminand().getLabel());
  }
  ...
  // }
  barSets.push_back(new QBarSet(QString::fromStdString(
      LocationDataset::instance().data[0].getDeterminand().getLabel())));
  for (const auto &sampleData : LocationDataset::instance().data) {
    ...
    if (sampleData.getDeterminand().getLabel() != temp.getLabel()) {
      ...
      PollutantDataset::instance().onPollutantChanged(
          QString::fromStdString(temp.getLabel()));
      getAverage();
...
void compliancePage::card() { // they are all the same pollutant
  ...
  for (const auto &data : PollutantDataset::instance().data) {
    ...
    // valueLabel[i]->setText(QString::asprintf("The measurement has %f%s more
    // than average", average - data.getResult().getValue(),
    // data.getDeterminand().getUnit()));
    // complianceLabel[i]->setText(QString::asprintf("The compliance indicates
    // %s-level state", getLevel(data))); cards->addWidget(cardHolder[i], row,
    // col); i++; currentIndex++;
    QVBoxLayout *cardLayout = new QVBoxLayout();
    QWidget *eachCardContainer = new QWidget();
    eachCardContainer->setLayout(cardLayout);
    QLabel *valueLabel = new QLabel(
        QString::asprintf("The measurement has %f%s more than average",
                          average - data.getResult().getValue(),
                          data.getDeterminand().getUnit().c_str()));
    QLabel *complianceLabel = new QLabel(QString::asprintf(
        "The compliance indicates %s-level state", getLevel(data).c_str()));
